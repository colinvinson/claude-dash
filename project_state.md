...
>
> **2026-05-16 — Finances tab v1 (0034):**
> New `/finances` route + BottomNav 7th tab (Wallet icon; tab width slimmed 50→44 to fit). Per Sir's directive: Business = MAKE money, Finances = MULTIPLY/spend/save. ChatGPT just shipped real bank/investment linking — Rowan deliberately doesn't compete on raw account aggregation; it owns the strategy layer ChatGPT can't because it lacks Sir's business/goal/life context.
> Schema (migration 0034): `wishlist_items` (title, price, kind ∈ {leverage, consumption}, status, optional goal_id + business_id backlinks) and `net_worth_snapshots` (monthly cash + investments + business_equity + debts, unique on user_id + snapshot_date). Hooks: useWishlist (entity-agnostic with optional scope filters), useNetWorth (upsert by date), useMoneyFlow (rolls up money_logs 0024 data — no new logging surface). Page order: Strategy hero → Wants → Cash flow → Net worth (collapsed by default to avoid vanity-metric pull). Strategy section opens Jarvis HUD with pre-built prompt including cash + MRR + wants total + the consumption-vs-leverage flag, asks "highest-EV move with what I have right now?" — that's the wedge no other tool can do. Wants list flags consumption > 2× leverage as cope. AddToWantsButton drops into BusinessDetail + GoalWidget so goals/businesses can assign items to the Finances tab with backlinks preserved. Jarvis context surfaces `finances.netWorth`, `finances.moneyFlow30d`, `finances.wants` (with the consumptionHeavy flag) so every chat about money is grounded in real numbers. Prompts.ts documents the 19/long-horizon/wealth>success framing — don't default to FIRE-blog tropes.
>
> **2026-05-16 — Linked external chats (0033):**
> New `linked_chats` table — references to external AI conversations (Claude.ai / ChatGPT / etc.) attached to either a `business_id` or a `goal_id` (both nullable, independent). Title + URL + source + summary + created_at. RLS, realtime, partial indexes per scope. `useLinkedChats({ businessId | goalId })` hook with add / update / archive. New `LinkedChats` component rendered both in BusinessDetail (below Marketing Experiments) and in GoalWidget's expanded view (below the bucket-move actions). Inline add form with source picker (Claude / ChatGPT / Jarvis / Other) and summary textarea. Context-builder surfaces `linkedChats[]` on both `context.businesses.items[]` and `context.longTermGoals[]` so Jarvis can say "you have a Claude chat from 3 days ago about pricing on SaaS v2" without being asked. The Rowan-native Jarvis chats stay flat-stream for now (no session boundaries) — that's a bigger refactor; v1 here is purely the external-link case which is the higher-leverage missing piece.
>
> **2026-05-16 — Marketing experiments + closed-loop agent learning (0032):**
> New `marketing_experiments` table (per-business): variant_label, variant_text, channel, posted_at, link, notes + nullable outcome metrics (impressions, clicks, conversions, revenue_attributed). useMarketingExperiments hook with add / updateMetrics / markPosted / archive. New `MarketingExperiments` component rendered in BusinessDetail between Agents and Activity — inline log form, expandable per-row metrics editor, top-performer ranked by CVR/CTR. Closed-loop wiring: `lib/businesses/agent-prompts.ts` now includes recent experiments + outcomes in every agent deploy prompt, so content/agents see what's been tried and what converted for Sir's audience BEFORE drafting new variants. BusinessAgents component fetches experiments via useMarketingExperiments and passes them through buildRunPrompt; the cron route does the same server-side in one batched query. Context-builder surfaces `recentExperiments[]` per business so Jarvis can read variant performance directly from chat context.
>
