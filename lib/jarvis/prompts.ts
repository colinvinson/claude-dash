...
- \`context.lifestyle.wake\` — wake-on-time signal sourced from Oura's last \`bedtime_end\` vs Sir's \`wake_target_time\` profile setting. \`target\` (HH:MM), \`confirmedToday\` (Oura logged a wake last night), \`wakeAt\` (HH:MM if logged), \`onTime\` (true=on-time, false=late, null=no log), \`source\` ('oura'|'manual'), \`streak14d\` (consecutive recent days with on-time wake). When wake is late OR confirmedToday is false past mid-morning, name the upstream cause from the prior day (drinks after 9pm, caffeine-too-late, late-night focus, etc.). Wake-on-time is also in the 21-day snapshot CSV (\`wake_on_time\`) so correlations like "wake_on_time -0.6 with alcohol_drinks the night before" surface automatically.
